...
function main() {
  ...
  var n = initVertexBuffers(gl); //配置纹理

  if (!initTextures(gl, n)) {}
}

function initVertexBuffers(gl) {
  var verticesTexCoords = new Float32Array([-0.5, 0.5, -0.3, 1.7, // 前 2 位是位置坐标， 后 2 位是纹理坐标
  -0.5, -0.5, -0.3, -0.2, 0.5, 0.5, 1.7, 1.7, 0.5, -0.5, 1.7, -0.2]);
  var n = 4; //顶点个数
  //创建缓冲区对象

  var vertexTexCoordBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexTexCoordBuffer); //向缓冲区对象中写入数据

  gl.bufferData(gl.ARRAY_BUFFER, verticesTexCoords, gl.STATIC_DRAW);
  var FSIZE = verticesTexCoords.BYTES_PER_ELEMENT; //获取attribute变量a_Position的存储位置

  var a_Position = gl.getAttribLocation(gl.program, "a_Position"); //将缓冲区对象分配给a_Position变量

  gl.vertexAttribPointer(a_Position, 2, gl.FLOAT, false, FSIZE * 4, 0); //连接a_Position变量与分配给他的缓冲区对象

  gl.enableVertexAttribArray(a_Position); //将纹理坐标分配给a_TexCoord并开启它
...
function loadTexture(gl, n, texture, u_Sampler, image) {
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, image); //将0号纹理传递给着色器

  gl.uniform1i(u_Sampler, 0); //设置canvas的背景色
